...
class GUI(QMainWindow):
    def __init__(self):
        ...
        self.trajectory_combo.addItem("Square Trajectory")
        ...
        self.m = None
        self.xbee_port = "COM5"  # Update with the appropriate port
        self.xbee = XBeeDevice(self.xbee_port, 9600)
        self.xbee.open()
    ...
    def show_details(self, index):
        ...
        if trajectory == "Straight Line":
            ...
            self.heading_label_display1.setVisible(True)
            self.heading_label_display2.setVisible(True)
        elif trajectory == "Square Trajectory":
            self.speed_edit.setEnabled(True)
            self.distance_edit.setEnabled(True)
            self.heading_edit.setEnabled(True)
            self.distance_label.setVisible(True)
        ...
        if trajectory == "Straight Line":
            ...
            for i in range(4):
                for _ in range(num_waypoints):
                    ...
                    current_lat += math.degrees(angular_distance) * math.cos(heading_rad)
                    current_lon += math.degrees(angular_distance) * math.sin(heading_rad)
                heading_rad=heading_rad+math.radians(90)
                i=i+1
            self.update_map()
    ...
    def update_map(self):
        ...
        for i, waypoint in enumerate(self.waypoints):
            ...
            if i == 0 or i == total_waypoints - 1:
                ...
                folium.Marker(location=[lat, lon], popup=f"Waypoint {i + 1}\nLatitude: {lat}\nLongitude: {lon}",icon=folium.Icon(color="red")).add_to(self.map)
    ...
    def move_to_next_waypoint(self):
        if self.current_waypoint_index < len(self.waypoints):
            ...
            delay = 2000
            icon = folium.CustomIcon(icon_image="rover.png", icon_size=(45,45))
    ...
    def deadreckoning(self):
        ...
        TOTAL_DISTANCE = float(self.distance_edit.text()) 
        NUM_WAYPOINTS = 5
        HEADING_ANGLES = 90
        initial_latitude = float(self.lat_edit.text())
        initial_longitude = float(self.lon_edit.text())
        initial_latitude_rad = math.radians(initial_latitude)
        initial_longitude_rad = math.radians(initial_longitude)
        current_latitude_rad = initial_latitude_rad
        current_longitude_rad = initial_longitude_rad
        ...
        with open('Accelerometer.csv', 'r') as file:
            ...
            for row in reader:
                ...
                x_acceleration = float(row[4])
                y_acceleration = float(row[3])
                z_acceleration = float(row[2])
                data.append((time_elapsed, x_acceleration, y_acceleration, z_acceleration))

        distance_per_waypoint = TOTAL_DISTANCE / NUM_WAYPOINTS

        for i in range(1, len(data)):
            time_elapsed = data[i][0] - data[i - 1][0]
            x_acceleration = data[i][1]
            y_acceleration = data[i][2]
            z_acceleration = data[i][3]
            ...
            if acceleration_magnitude > ACCELERATION_THRESHOLD:
                distance = 0.5 * acceleration_magnitude * (time_elapsed ** 2)
                distance_traveled += distance
                if distance_traveled >= distance_per_waypoint:
                    remaining_distance = distance_per_waypoint - (distance_traveled - distance)
                    longitude_change = math.degrees(remaining_distance / (EARTH_RADIUS * math.cos(current_latitude_rad)))
                    current_longitude_rad += math.radians(longitude_change)
                    self.waypoints1.append((math.degrees(current_latitude_rad), math.degrees(current_longitude_rad)))
                    distance_traveled = 0.0
        ...
        for i in range(1, len(data)):
            ...
            if acceleration_magnitude > ACCELERATION_THRESHOLD:
                ...
                if distance_traveled >= distance_per_waypoint:
                    remaining_distance = distance_per_waypoint - (distance_traveled - distance)
                    latitude_change = math.degrees(remaining_distance / EARTH_RADIUS)
                    current_latitude_rad -= math.radians(latitude_change)
                    self.waypoints1.append((math.degrees(current_latitude_rad), math.degrees(current_longitude_rad)))
                    distance_traveled = 0.0
                if distance_traveled >= TOTAL_DISTANCE:
                    break
        ...
        for i in range(1, len(data)):
            ...
            if acceleration_magnitude > ACCELERATION_THRESHOLD:
                ...
                if distance_traveled >= TOTAL_DISTANCE:
                    break
        self.map_view.page().runJavaScript("map.remove();")
        map_center = [initial_latitude, initial_longitude]
        folium.Marker([initial_latitude, initial_longitude], popup="Latitude: " + str(initial_latitude) + ", Longitude: " + str(initial_longitude)).add_to(self.map)
        for i, waypoint in enumerate(self.waypoints1):
            folium.Marker(waypoint,popup="Latitude: " + str(waypoint[0]) + ", Longitude: " + str(waypoint[1])).add_to(self.map)
        folium.PolyLine([map_center] + self.waypoints1, color='green', weight=2.5).add_to(self.map)
    ...
    def mark_gps_locations_on_map(self):
        ...
        if xbee_message is not None:
            gps_data = xbee_message.data.decode('utf-8')
            ...
            try:
                ...
                print("Invalid GPS data format:", gps_data)
                return

            folium.Marker(location=[lat, lon],
                          popup="Latitude: " + str(lat) + ", Longitude: " + str(lon),
                          icon=folium.Icon(icon="cloud")).add_to(self.map)
            ...
            self.lat_label_display1.setText("\t\tCurrent Latitude:  ")
            self.lat_label_display2.setStyleSheet("border: 2px solid black; padding: 2px;")
            self.lat_label_display2.setText("<font color='blue'>" + str(lat) + "</font>")
            self.lon_label_display1.setText("\t\tCurrent Longitude:  ")
            self.lon_label_display2.setStyleSheet("border: 2px solid black; padding: 2px;")
            self.lon_label_display2.setText("<font color='blue'>" + str(lon) + "</font>")
...
if __name__ == "__main__":
    ...
    app.setStyleSheet("QPushButton { background-color: #4CAF50 !important; color: black;font-weight: bold;}"
                      ...
                      "QComboBox { background-color: #F0F0F0;color: blue;font-weight: bold; }"
